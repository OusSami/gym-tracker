...
export default async function handler(req, res) {
  ...
  if (req.method === 'PUT') {
    ...
    if (type === 'add_exercise') {
      // Deduplicate: if same exercise name was added in last 10 seconds, return existing
      const tenSecsAgo = new Date(Date.now() - 10000).toISOString()
      const { data: recent } = await sb.from('exercises')
        .select('*').eq('session_id', sessionId).eq('name', exercise.name)
        .gte('created_at', tenSecsAgo).order('created_at', { ascending: false }).limit(1)
      if (recent?.length) return res.status(200).json({ exercise: recent[0] })
      const { data: exRow, error: exErr } = await sb.from('exercises')
    ...
    // Batch-save all sets for an exercise (safety net at exercise finish)
    if (type === 'batch_sets') {
      const { sets } = req.body
      if (!sets?.length || !exerciseId) return res.status(400).json({ error: 'Missing sets or exerciseId' })
      // Delete existing sets for this exercise and re-insert all
      // This is idempotent: safe to call multiple times
      await sb.from('sets').delete().eq('exercise_id', exerciseId)
      ...
      const { error } = await sb.from('sets').insert(toInsert)
      if (error) return res.status(500).json({ error: error.message })
