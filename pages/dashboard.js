...
export default function Dashboard() {
  ...
  const [addSetExId, setAddSetExId] = useState(null)  // inline add-set in history
  const [addSetVal, setAddSetVal] = useState({w:'',r:''}) // weight/reps for inline add
  ...
  const addSetInline = async (sessionId, exId, lastSet) => {
    const w = addSetVal.w === '' ? null : parseFloat(addSetVal.w)
    const r = parseInt(addSetVal.r)
    if (w === null || isNaN(w) || !r || r <= 0) return
    setSaving(true)
    await fetch('/api/sessions', { method:'PUT', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ type:'add_set', sessionId, exerciseId:exId, set:{ weight_kg:w, reps:r } }) })
    await reload(user.id)
    setAddSetExId(null); setAddSetVal({w:'',r:''}); setSaving(false)
  }
                      ...
                      <div style={{marginTop:16,borderTop:'1px solid #1a1a1a',paddingTop:16}} onClick={e=>e.stopPropagation()}>
                        ...
                        {s.exercises?.map(ex=>(
                          <div key={ex.id} style={{marginBottom:14}}>
                            ...
                            {/* + Add Set */}
                            {addSetExId === ex.id ? (
                              <div style={{marginTop:6,padding:'10px 12px',background:'rgba(200,255,0,0.04)',border:'1px solid rgba(200,255,0,0.15)',borderRadius:10}}>
                                <div style={{fontSize:'.58rem',fontWeight:700,letterSpacing:1.5,color:'rgba(200,255,0,0.5)',marginBottom:8}}>ADD SET {(ex.sets?.length||0)+1}</div>
                                <div style={{display:'flex',gap:8,alignItems:'center',marginBottom:8}}>
                                  <div style={{flex:1}}>
                                    <div style={{fontSize:'.6rem',color:'#555',fontWeight:600,marginBottom:3}}>WEIGHT (kg)</div>
                                    <input type="number" inputMode="decimal"
                                      placeholder={ex.sets?.length ? String(ex.sets[ex.sets.length-1].weight_kg) : '0'}
                                      value={addSetVal.w} onChange={e=>setAddSetVal(v=>({...v,w:e.target.value}))}
                                      style={{width:'100%',background:'#141414',border:'1px solid #2a2a2a',color:'#f0f0f0',padding:'9px 12px',borderRadius:8,fontSize:'.9rem',outline:'none'}}/>
                                  </div>
                                  <div style={{flex:1}}>
                                    <div style={{fontSize:'.6rem',color:'#555',fontWeight:600,marginBottom:3}}>REPS</div>
                                    <input type="number" inputMode="numeric"
                                      placeholder={ex.sets?.length ? String(ex.sets[ex.sets.length-1].reps) : '8'}
                                      value={addSetVal.r} onChange={e=>setAddSetVal(v=>({...v,r:e.target.value}))}
                                      style={{width:'100%',background:'#141414',border:'1px solid #2a2a2a',color:'#f0f0f0',padding:'9px 12px',borderRadius:8,fontSize:'.9rem',outline:'none'}}/>
                                  </div>
                                </div>
                                <div style={{display:'flex',gap:8}}>
                                  <button onClick={()=>addSetInline(s.id, ex.id)} disabled={saving || addSetVal.w===''||!addSetVal.r}
                                    style={{flex:1,padding:'9px',background:'#c8ff00',border:'none',borderRadius:8,fontFamily:"'Space Grotesk',sans-serif",fontWeight:800,fontSize:'.82rem',color:'#080808',cursor:'pointer'}}>
                                    {saving ? 'SAVING...' : '+ SAVE SET'}
                                  </button>
                                  <button onClick={()=>{setAddSetExId(null);setAddSetVal({w:'',r:''})}}
                                    style={{padding:'9px 14px',background:'transparent',border:'1px solid #2a2a2a',borderRadius:8,color:'#555',cursor:'pointer',fontSize:'.82rem'}}>
                                    Cancel
                                  </button>
                                </div>
                              </div>
                            ) : (
                              <button onClick={()=>{setAddSetExId(ex.id);setAddSetVal({w:'',r:''})}}
                                style={{marginTop:6,width:'100%',padding:'8px',background:'rgba(200,255,0,0.04)',border:'1px dashed rgba(200,255,0,0.15)',borderRadius:8,color:'rgba(200,255,0,0.5)',cursor:'pointer',fontSize:'.78rem',fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,letterSpacing:.5}}>
                                + ADD SET
                              </button>
                            )}
                          </div>
