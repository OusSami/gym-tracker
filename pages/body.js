...
export default function BodyAnalysis() {
  ...
  const loadImg = (file, side) => {
    ...
    setErr('')
    // Compress to max 1000px / 75% quality to avoid Vercel 15MB body limit
    const img = new Image()
    ...
    img.onload = () => {
      ...
      canvas.toBlob(blob => {
        ...
        fr.onload = e => {
          const d = e.target.result; const b64 = d.split(',')[1]
          if (side === 'front') { setFrontPreview(d); setFrontB64(b64); setFrontMime('image/jpeg') }
          else { setBackPreview(d); setBackB64(b64); setBackMime('image/jpeg') }
        }
    ...
    img.onerror = () => {
      ...
      fr.onload = e => {
        const d = e.target.result; const b64 = d.split(',')[1]
        if (side === 'front') { setFrontPreview(d); setFrontB64(b64); setFrontMime(file.type||'image/jpeg') }
        else { setBackPreview(d); setBackB64(b64); setBackMime(file.type||'image/jpeg') }
      }
  ...
  const analyze = async () => {
    ...
    const frontBytes = Math.round((frontB64?.length||0) * 0.75)
    const backBytes = Math.round((backB64?.length||0) * 0.75)
    if (frontBytes + backBytes > 9000000) { setErr('Photos too large. Please retake with lower camera resolution.'); return }
