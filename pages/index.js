...
export default function App() {
  ...
  const exCreating = useRef(false) // true while add_exercise API call is in flight
  ...
  const autosave = async (extraDone, extraCur) => {
    ...
    // If live session is active, skip autosave - exercises are already saved live
  ...
  const logSet = async () => {
    ...
    // Save set to Supabase - wait for exercise creation if in flight
    if (liveExerciseId.current && !exCreating.current) {
      try {
        await fetch('/api/sessions', { method:'PUT', headers:{'Content-Type':'application/json'},
          body: JSON.stringify({ type:'add_set', sessionId:liveSessionId.current, exerciseId:liveExerciseId.current,
            set:{ weight_kg:w, reps:r, duration:activeDur, total_duration:totalDur } }) })
      } catch(e) { console.error('Set save failed (data still in localStorage):', e) }
    } else if (exCreating.current) {
      // Exercise still being created - retry after short delay
      setTimeout(async () => {
        if (liveExerciseId.current) {
          try {
            await fetch('/api/sessions', { method:'PUT', headers:{'Content-Type':'application/json'},
              body: JSON.stringify({ type:'add_set', sessionId:liveSessionId.current, exerciseId:liveExerciseId.current,
                set:{ weight_kg:w, reps:r, duration:activeDur, total_duration:totalDur } }) })
          } catch(e) { console.error('Set save retry failed:', e) }
        }
      }, 2000)
    }
  }
  ...
  const finishEx = async () => {
    ...
    if (next) {
      ...
      setCset({w:'',r:''}); setScreen(S.LOGGING)
      // Create next exercise in Supabase BEFORE showing logging screen
      liveExerciseId.current = null // clear so logSet won't fire for wrong exercise
      exCreating.current = true
    ...
    // Batch-save current exercise's sets as safety net (in case individual saves failed)
    if (liveExerciseId.current && cur.sets?.length && liveSessionId.current) {
      try {
        await fetch('/api/sessions', { method:'PUT', headers:{'Content-Type':'application/json'},
          body: JSON.stringify({ type:'batch_sets', sessionId:liveSessionId.current,
            exerciseId:liveExerciseId.current,
            sets: cur.sets.map((s,i)=>({ weight_kg:s.weight||s.weight_kg||0, reps:s.reps||0,
              set_number:i+1, duration:s.duration||0, total_duration:s.total_duration||0 })) }) })
      } catch(e) { console.error('Batch set save failed:', e) }
    }
